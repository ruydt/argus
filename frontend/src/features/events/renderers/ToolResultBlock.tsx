...
export function ToolResultBlock({
  ...
  return (
    <>
      {stdout && (
        <div
          className="group/eblock mt-2 overflow-hidden rounded-[6px] border border-foreground/[0.08] bg-foreground/[0.04] text-[0.75rem] select-text"
          data-event-drag-ignore
        >
          <div className="flex items-center justify-between px-3 py-1 border-b border-foreground/[0.08]">
            <div className="flex items-center gap-3">
              <strong className="text-muted-foreground text-[0.7rem]">stdout</strong>
              {durationMs != null && durationMs > 0 && (
                <span className="text-muted-foreground text-[0.65rem]">{durationMs}ms</span>
              )}
            </div>
            <CopyIconButton
              text={stdout}
          ...
          <pre className="px-3 py-2 mb-0 whitespace-pre-wrap break-words text-[0.73rem] text-muted-foreground max-h-[240px] overflow-y-auto font-[inherit]">
