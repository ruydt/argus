...
export function TraceBlock({
  ...
  const ticks =
    rootDuration > 0 ? [0, 0.25, 0.5, 0.75, 1].map((pct) => formatTimeAxis(pct * rootDuration)) : []
    ...
    <div style={{ borderBottom: '2px solid #111' }}>
      ...
      {expanded &&
        node.children.map((child, idx) => {
          ...
          const childStartMs =
            new Date(child.session.started_at).getTime() - new Date(session.started_at).getTime()
          const leftPct =
            rootDuration > 0 && Number.isFinite(childStartMs)
              ? Math.max(0, (childStartMs / rootDuration) * 100)
              : 0
          const widthPct =
            rootDuration > 0 && Number.isFinite(childStartMs)
              ? Math.min(100 - leftPct, (childDuration / rootDuration) * 100)
              : 0
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div
                    style={{
                      fontSize: 9,
                      color: childSelected ? '#c4b5fd' : '#888',
                      fontFamily: '"SF Mono", "Fira Code", monospace',
                      overflow: 'hidden',
                      textOverflow: 'ellipsis',
                      whiteSpace: 'nowrap',
