...
export function ScriptViewerModal({ title, open, onOpenChange, load }: ScriptViewerModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[90vw] !max-w-5xl border border-foreground/15 bg-popover sm:!max-w-5xl">
        <DialogHeader>
        ...
        {error ? (
          ...
          <pre className="max-h-[70vh] overflow-auto rounded-md bg-foreground/[0.04] p-3 font-mono text-[0.72rem] leading-relaxed text-muted-foreground">
            {body}
          </pre>
        )}
      </DialogContent>
